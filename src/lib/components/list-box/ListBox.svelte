...
<div class="relative w-full dropdown {className}" bind:this={dropdownElement}>
	<button
		...
		class="inline-flex items-center justify-between bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-100 border-2 border-slate-200 dark:border-slate-600 w-full px-3 py-2 rounded-md cursor-pointer focus:outline-hidden focus-visible:ring-4 focus-visible:ring-opacity-75 focus-visible:ring-indigo-500 dark:focus-visible:ring-indigo-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors duration-200 disabled:cursor-not-allowed disabled:hover:bg-slate-100 dark:disabled:hover:bg-slate-800"
	>
	...
	{#if $expanded}
		...
		>
			{#each items as item}
				{#if isCategory(item)}
					<li class="bg-indigo-100 dark:bg-indigo-900/20 px-3 py-1 text-sm font-semibold text-indigo-800 dark:text-indigo-200">{item.group}</li>
					{#each item.items as subItem}
						<li>
							<button
								...
								class="flex items-center w-full px-3 py-2 text-slate-900 dark:text-slate-100 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors duration-200"
							>
								{#if String(selectedValue) === String(subItem.value)}
									<span class="w-3 h-3 mr-2 rounded-full bg-indigo-500 dark:bg-indigo-400"></span>
								{:else}
